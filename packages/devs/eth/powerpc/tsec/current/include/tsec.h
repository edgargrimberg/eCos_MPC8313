...
struct tsec_eth_info {
    ...
    volatile struct tsec_bd       *tnext, *rnext;     // Next descriptor to check for interrupt
    ...
    cyg_uint8                         tsec_if;                // TSEC1, TSEC2 etc...    
    unsigned char                   *init_rxbufs;        // Initial base pointer of RX buffers
    unsigned char                   *init_txbufs;        // Initial base pointer of TX buffers
    struct tsec_bd                   *init_rxring;        // Initial base pointer of RX ring
    struct tsec_bd                   *init_txring;	     // Initial base pointer of TX ring
...
#endif                      
    tsec_mii_info_t                 *mii;                    // Select MII interface
    ...
    tsec_smp_stats_t             smp_stats[CYGPKG_HAL_SMP_CPU_MAX];
#endif
}; 
...
static void tsec_phy_put_reg(tsec_mii_info_t *, int , int , unsigned short );
static bool tsec_phy_get_reg(tsec_mii_info_t *, int reg, int phy, unsigned short *val);

#ifdef CYGARC_REG_IMM_TSEC1
static void tsec1_phy_put_reg(int , int , unsigned short );
static bool tsec1_phy_get_reg(int , int , unsigned short *);
#endif

#ifdef CYGARC_REG_IMM_TSEC3
static void tsec3_phy_put_reg(int , int , unsigned short );
static bool tsec3_phy_get_reg(int , int , unsigned short *);
#endif

#define TSEC_MAX_INTERFACE 4
