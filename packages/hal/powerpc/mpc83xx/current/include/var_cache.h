...
#define HAL_DCACHE_DISABLE()                \
    ...
    {\
        ...
        tmp2 = *((volatile cyg_uint32 *) tmp1);      \
        tmp2 = *((volatile cyg_uint32 *) tmp1);      \
        asm volatile ("dcbf 0,%0;sync;" : : "r" (tmp1));		\
     }\
    asm volatile (                          \
    	"isync;"                            \
		"sync;"\
...
//
//
// Hmmm...  dcbf 0,rX is spooky as it dissassembles as dcbf r0,rX...
// is it disassembly that is broken??
//
// http://publib.boulder.ibm.com/infocenter/systems/index.jsp?topic=/com.ibm.aix.aixassem/doc/alangref/dcbi.htm
#define HAL_DCACHE_FLUSH( _base_ , _size_ )                     \
    ...
    cyg_int32 __zero = 0;                    \
    while (__size > 0) {                                        \
        asm volatile ("dcbf %0,%1;sync;" : : "r" (__zero), "r" (__base));      \
        __base += HAL_DCACHE_LINE_SIZE;                         \
        __size -= HAL_DCACHE_LINE_SIZE;                         \
    }                                                           \
    CYG_MACRO_END

//Data cache flushing is accomplished by filling the data cache with
//known data and performing a flash invalidate or a series of
//dcbf instructions that force a flush and
//invalidation of the data cache block.
#define HAL_DCACHE_FLUSH_ALL() 									\
    CYG_MACRO_START												\
    register cyg_uint32 base = 0; 								\
    register cyg_uint32 index = 0;								\
    register cyg_uint32 tmp ;									\
																\
	for(base = 0; base < HAL_DCACHE_SIZE; base += HAL_DCACHE_LINE_SIZE)	\
    {															\
		for(index = 0; index < 0x1000; index ++)					\
			tmp = *(volatile cyg_uint32 *)(base);				\
	}															\
	for(base = 0; base < HAL_DCACHE_SIZE; base += HAL_DCACHE_LINE_SIZE)	\
    {															\
    	asm volatile ("dcbf 0,%0;sync;" : : "r" (base));		\
	}															\
    CYG_MACRO_END
...
// http://publib.boulder.ibm.com/infocenter/systems/index.jsp?topic=/com.ibm.aix.aixassem/doc/alangref/dcbi.htm
//
// Yuk! Optimize? do we need to invalidate + flush or is invalidate
// sufficient?
